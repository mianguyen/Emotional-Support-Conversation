...
class ESC(BaseFramework):
    def __init__(self, encoder=None, decoder=None, toker=None, **kwargs):
        ...
        assert self.expand_vocab_size in [9, 10]
    ...
    def predict_strategy(self, logits, encoded_info):
        assert not self.training
        tgt_strat_id = encoded_info.get('tgt_strat_id', None)
        logits = logits[:, 0, -8:].contiguous()
        #print('strat_dist', F.softmax(logits, dim=-1)[0])
    
        if tgt_strat_id is not None:
            pred = tgt_strat_id
        else:
            if SAMPLE:
    ...
    def generate(
        ...
        **kwargs
        ...
        tgt_input_ids = torch.cat([tgt_input_ids, encoded_info['pred_strat_id'].unsqueeze(1) + len(self.toker) - 8], dim=-1)
