...
def featurize(
    ...
    response, max_tgt_len, tgt_strat_id,
):
    assert len(context) == len(src_strat_ids)
    src_input_ids = [[st] + ctx + [eos] for ctx, st in zip(context, src_strat_ids)]
    src_input_ids = sum(src_input_ids, [])[:-1]
    src_input_ids = src_input_ids[-max_src_len:]
    
    tgt_label_ids = ([tgt_strat_id] + response + [eos])[:max_tgt_len + 1]
...
def convert_data_to_inputs(data, toker: PreTrainedTokenizer, **kwargs):
    ...
    assert len(toker) == toker.vocab_size + 1 + 8
    ...
    for i in range(len(dialog)):
        ...
        if dialog[i]['speaker'] == 'usr':
            ...
            strat_id = len(toker) - 8 + dialog[i]['strategy']
...
def convert_inputs_to_features(inputs, toker, **kwargs):
    ...
    for i in range(len(inputs)):
        ...
        feat = featurize(
            ...
            ipt['response'], max_tgt_len, ipt['tgt_strat_id'],
        )
...
class FeatureDataset(Dataset):
    ...
    def collate(features: List[InputFeatures], toker: PreTrainedTokenizer):
        ...
        tgt_strat_id = tgt_label_ids[:, 0] - len(toker) + 8
...
def prepare_infer_batch(features, toker, interact=None):
    ...
    if interact is None and GOLDEN_TRUTH:
        other_res['cls_strat_id'] = res.get('tgt_strat_id')
    else:
        other_res['cls_strat_id'] = res.pop('tgt_strat_id')
